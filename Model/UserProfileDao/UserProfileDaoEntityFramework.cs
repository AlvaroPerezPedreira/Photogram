...
namespace Es.Udc.DotNet.PracticaMaD.Model.UserProfileDao
{
        ...
        GenericDaoEntityFramework<UserProfile, Int64>, IUserProfileDao
    {
        ...
        public List<UserProfile> findSeguidores(UserProfile user, int startIndex, int count)
        {
            var result = user.Seguidor.Skip(startIndex).Take(count).ToList();
            return result;
        }

        public List<UserProfile> findSeguidos(UserProfile user, int startIndex, int count)
        {
            var result = user.Seguido.Skip(startIndex).Take(count).ToList();
            return result;
        }

        public Boolean isUserFollowingUser(UserProfile user, UserProfile followerToCheck)
        {
            return followerToCheck.Seguidor.Any(seguidor => seguidor.usrId == user.usrId);
        }
