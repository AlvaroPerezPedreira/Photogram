...
namespace Es.Udc.DotNet.PracticaMaD.UnitTest
{
    ...
    public class IUserServiceTest
    {
        ...
        [TestMethod]
        public void UserFollowUnfollowUser()
        {
            using (TransactionScope scope = new TransactionScope())
            {
                var userIdSeguidor = userService.RegisterUser("a1", "b1",
                    new UserProfileDetails("c1", "d1", "a1@udc.es", language, country));

                var userIdSeguido = userService.RegisterUser("a2", "b2",
                    new UserProfileDetails("c2", "d2", "a2@udc.es", language, country));

                userService.SeguirUsuario(userIdSeguido, userIdSeguidor);

                var userProfile = userProfileDao.Find(userIdSeguido);
                var userProfile2 = userProfileDao.Find(userIdSeguidor);

                Assert.AreEqual(userProfile.Seguidor.ToString(), userProfile2.Seguido.ToString());

                userService.Unfollow(userIdSeguido, userIdSeguidor);

                Assert.AreEqual(0, userProfile.Seguidor.Count);
                Assert.AreEqual(0, userProfile2.Seguido.Count);
            }
        }

        [TestMethod]
        public void TestFollowFindAllSeguidoresAndPaginado()
        {
            using (TransactionScope scope = new TransactionScope())
            {
                long userId1 = userService.RegisterUser("a", "a",
                    new UserProfileDetails("a", "a", "a@udc.es", language, country));

                UserProfile user1 = userProfileDao.Find(userId1);

                long userId2 = userService.RegisterUser("b", "b",
                    new UserProfileDetails("b", "b", "b@udc.es", language, country));

                UserProfile user2 = userProfileDao.Find(userId2);

                long userId3 = userService.RegisterUser("c", "c",
                    new UserProfileDetails("c", "c", "c@udc.es", language, country));

                UserProfile user3 = userProfileDao.Find(userId3);

                long userId4 = userService.RegisterUser("d", "d",
                    new UserProfileDetails("d", "d", "d@udc.es", language, country));

                UserProfile user4 = userProfileDao.Find(userId4);

                userService.SeguirUsuario(userId2, userId1);
                userService.SeguirUsuario(userId2, userId3);
                userService.SeguirUsuario(userId2, userId4);
                UsersBlock seguidores = userService.FindAllSeguidores(user2, 0, 2);
                Assert.AreEqual(2, seguidores.Users.Count);
                Assert.AreEqual(user1, seguidores.Users[0]);
                Assert.AreEqual(user3, seguidores.Users[1]);
                Assert.IsTrue(seguidores.ExistMoreUsers);

                userService.Unfollow(userId2, userId1);
                userService.Unfollow(userId2, userId3);
                userService.Unfollow(userId2, userId4);
            }
        }

        [TestMethod]
        public void TestFollowFindAllSeguidosAndPaginado()
        {
            using (TransactionScope scope = new TransactionScope())
            {
                long userId1 = userService.RegisterUser("a", "a",
                ...
                userService.SeguirUsuario(userId2, userId3);
                userService.SeguirUsuario(userId2, userId4);
                UsersBlock seguidos = userService.FindAllSeguidos(user2, 0, 2);
                Assert.AreEqual(2, seguidos.Users.Count);
                Assert.AreEqual(user1, seguidos.Users[0]);
                Assert.AreEqual(user3, seguidos.Users[1]);
                Assert.IsTrue(seguidos.ExistMoreUsers);
                userService.Unfollow(userId2, userId1);
                userService.Unfollow(userId2, userId3);
                userService.Unfollow(userId2, userId4);
            }
